<script lang="ts">
	...
	let term = $state(collection.term);
	...
	const onSubmit = async (e: Event) => {
		const series_ =
			series.find((x) => x.index === seriesName) ?? createSeriesType(null, seriesName);
	...
	<form onsubmit={onSubmit}>
		...
		<div class="input-field">
			...
			<input
				...
				list="C9C3E8B4-6C2E-4F5F-9A2E-2C7A6B8B6C7D"
				onchange={onChangeSeriesOrIssue}
			/>
		</div>
		<div class="input-field">
