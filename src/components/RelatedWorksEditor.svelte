<script lang="ts">
	...
	type PropsType = {
		...
		relatedWorks: RelatedWorkType[];
		works: WorkType[];
		callback: (links: RelatedWorkType[]) => void;
	};
	type ItemType = {
		orderNo: number;
		workIndex: string;
	...
	const callCallback = () => {
		const t: RelatedWorkType[] = items.map((x) => {
            const work = works.find((z) => z.index === x.workIndex);
			return ({
				relatedType,
				subType,
				relatedId,
				workId: work?.id ?? null,
                workName: work ? work.title : x.workIndex,
				description: x.description
			});
		});
		callback?.(t);
	...
	<div class="input-field">
		...
		<div class="person-data" data-order-no={item.orderNo}>
			<input
				name="workIndex"
				type="text"
				bind:value={item.workIndex}
				list={uuid}
				onchange={onChangeRelatedWorksIndex}
			/>
