...
// 関連シリーズの登録
export async function add(db: pkg.Database, relatedType: string,  relatedId: number, relatedCollection: RelatedCollectionType) {
    return new Promise<void>(async (resolve, reject) => {
        try {
            ...
            const sql = 'INSERT INTO related_collections (relatedType, relatedId, collectionId, description) VALUES (?, ?, ?, ?)';
            const params = [relatedType, relatedId, relatedCollection.collectionId, relatedCollection.description];
...
export async function deleteAll(db: pkg.Database, relatedType: string, relatedId: number) {
    return new Promise<void>((resolve, reject) => {
        db.run('DELETE FROM related_collections WHERE relatedType = ? AND relatedId = ?', [relatedType, relatedId], function (error) {
            if (error) {
                reject(error);
            } else {
                resolve();
