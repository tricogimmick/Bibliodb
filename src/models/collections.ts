...
export async function update(db: pkg.Database, collection: CollectionType) {
	...
	const [sql, params] =
		collection.id === null
			? [
					'INSERT INTO collections (title, seriesId, term, issue, collectionType, description, note) VALUES (?, ?, ?, ?, ?, ?, ?)',
					[
						collection.title,
						collection.seriesId,
						collection.term,
			...
			: [
					'UPDATE collections SET title = ?, seriesId = ?, term = ?, issue = ?, collectionType = ?, description = ?, note = ? WHERE id = ?',
					[
						collection.title,
						collection.seriesId,
						collection.term,
